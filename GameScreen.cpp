...
GameScreen::GameScreen()
{
	...
	mCamera->SetAsPerspective(Vector3D(-2.5f, -30.0f, -200.0f), Vector3D(0.0f, 0.0f, 0.0f), Vector3D(0.0f, 1.0f, 0.0f),
		aNear, aFar, aFieldOfView, aAspect);

	//mCamera->SetAsPerspective(Vector3D(0.0f, -30.0f, -90.0f), Vector3D(0.0f, 0.0f, 0.0f), Vector3D(0.0f, 1.0f, 0.0f),
	//							aNear, aFar, aFieldOfView, aAspect);
...
void GameScreen::Update(float deltaTime, SDL_Event e)
{
	...
	{
		myTimer->StopTimer();
		myTimer->SetTimer(myTimer->GetTimer());
	}
	...
	case SDL_KEYDOWN:
		...
		{
			case SDLK_RETURN:
				mCamera->SetAsPerspective(Vector3D(0.0f, 0.0f, -100.0f), Vector3D(0.0f, 0.0f, 0.0f), Vector3D(0.0f, 1.0f, 0.0f),
					aNear, aFar, aFieldOfView, aAspect);
				break;
	/*
			case SDLK_0:
