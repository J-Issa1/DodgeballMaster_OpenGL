...
const float kMoveDistance = 2.0f;
...
void Camera::Render()
{
	...
	//v_Quaternion->MatrixToQuaternion(instance, m_Quaternion);
	//v_Quaternion->RotateVector(m_Quaternion, m_Quaternion->vec, )

	// Pitch
	glRotatef(m_fPitchAngle, m_vRight.x, 0.0f, 0.0f);
...
void Camera::SetPosition(Vector3D newCamPos)
{
	m_vPosition = newCamPos;
}
